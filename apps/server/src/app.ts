...
if (config.FULLSTACK_RELAY_ENV === 'production') {
  app.proxy = true;
}
...
app.use(
  cors({
    maxAge: 86400,
    credentials: true,
    origin: config.FULLSTACK_RELAY_ENV === 'production' ? 'https://app.com' : '*',
    secureContext: config.FULLSTACK_RELAY_ENV !== 'development',
  }),
);
