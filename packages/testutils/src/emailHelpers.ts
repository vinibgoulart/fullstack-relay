...
export const assertFullstackRelayEmail = (email: SesCall, subject = 'Fullstack-relay') => {
  expect(getEmailReplyTo(email)[0]).toBe('naoresponda@fullstack-relay.com.br');
  expect(getEmailSource(email).includes('naoresponda@fullstack-relay.com.br')).toBe(
    true,
  );
